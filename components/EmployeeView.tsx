        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
          <div className="bg-emerald-950 rounded-[2.5rem] p-7 text-white flex justify-between items-center shadow-2xl relative overflow-hidden">
            ...
            <div className="flex gap-2 relative z-10">
              <button onClick={() => setPeriod(p => p.part === 2 ? { ...p, part: 1 } : { year: p.month === 1 ? p.year - 1 : p.year, month: p.month === 1 ? 12 : p.month - 1, part: 2 })} className="w-11 h-11 flex items-center justify-center bg-white/10 rounded-2xl hover:bg-white/20 active:scale-90 transition-all text-white">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M15 18l-6-6 6-6" /></svg>
              </button>
              <button onClick={() => setPeriod(p => p.part === 1 ? { ...p, part: 2 } : { year: p.month === 12 ? p.year + 1 : p.year, month: p.month === 12 ? 1 : p.month + 1, part: 1 })} className="w-11 h-11 flex items-center justify-center bg-white/10 rounded-2xl hover:bg-white/20 active:scale-90 transition-all text-white">
          ...
          <div className="grid gap-3">
            {currentPeriodDates.map(date => {
              ...
              return (
                <div key={date} className={`bg-white p-6 rounded-[2rem] border-2 transition-all duration-300 ${isApproved ? 'border-emerald-600 bg-emerald-50/5' : isWorking ? 'border-lime-500 bg-lime-50/10 shadow-lg' : 'border-gray-50'}`}>
      ...
      {activeTab === 'PAYROLL' && (
        <div className="space-y-6 animate-in fade-in">
          <div className="bg-white rounded-[3rem] p-12 border border-gray-50 text-center shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-[11px] font-black text-lime-500 uppercase tracking-[0.4em] mb-4 relative z-10">Total Verified Payout</p>
            <h2 className="text-4xl sm:text-5xl font-black text-emerald-950 tracking-tighter relative z-10 mb-8 overflow-hidden break-all px-4">
